...
export default function MasterProducts() {
  ...
  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from("master_products").select("*").order("display_order");
    setItems(data ?? []);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const add = async () => {
    ...
    const { error } = await supabase.from("master_products").insert({ name: name.trim(), display_order: next });
    if (error) { toast.error(error.message); return; }
    setName(""); toast.success("Product added"); load();
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Manage the global product list available for assignment.</p>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-base">Add new product</CardTitle></CardHeader>
        <CardContent>
          <div className="flex gap-2">
            <Input placeholder="Product name" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} />
